...
public class $ {
    ...
    /// :return An array with unique values.
    public class func uniq<T, U: Hashable>(array: [T], by condition: (T) -> U) -> [T] {
        var result: [T] = []
...
public class Chain<C> {
    ...
    ///
    /// :param condition called per iteration
    /// :return The Wrapper objject.
    public func uniq<U: Hashable>(function: C -> U) -> Chain {
        return self.queue {
            return Wrapper($.uniq($0.value, by: function))
        }
    }
